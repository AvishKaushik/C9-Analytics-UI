...
export default function DraftTeamSearch({ selectedTeam, onSelectTeam, placeholder = "Search LoL team..." }: Props) {
  ...
  const searchMutation = useMutation({
    mutationFn: () => draftApi.searchTeams(searchQuery, 20),
  })
  ...
  const handleInputFocus = () => {
    if (searchQuery.length >= 2 && searchMutation.data?.teams?.length > 0) {
      setShowResults(true)
    }
  }

  const results = searchMutation.data?.teams || []
